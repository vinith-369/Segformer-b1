...
class DWConv(nn.Module):
    """Depth-wise convolution used inside the MLP of each transformer block."""
...
class Mlp(nn.Module):
    """MLP with depth-wise conv (Mix-FFN from SegFormer paper)."""
...
class Block(nn.Module):
    """Transformer block: LayerNorm -> Attention -> LayerNorm -> Mix-FFN."""
...
class OverlapPatchEmbed(nn.Module):
    """Overlapping patch embedding using convolution."""
...
class MixVisionTransformer(nn.Module):
    """Mix Vision Transformer (MiT) encoder backbone.

    Produces multi-scale features at 1/4, 1/8, 1/16, 1/32 resolution.
    """
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000,
        ...
        # Overlapping patch embeddings for each stage
